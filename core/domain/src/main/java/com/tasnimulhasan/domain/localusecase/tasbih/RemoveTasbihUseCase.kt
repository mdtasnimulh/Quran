...
class RemoveTasbihUseCase @Inject constructor(
    private val repository: TasbihRepository
) : RoomSuspendableUseCaseNonReturn<RemoveTasbihUseCase.Params> {

    data class Params(
        val item: TasbihItem
    )

    override suspend fun invoke(params: Params) = repository.deleteTasbih(params.item)
}